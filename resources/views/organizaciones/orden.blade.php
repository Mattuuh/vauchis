...
$(document).ready(function () {
    ...
    $('#btn-guardar-orden').on('click', function () {
        ...
        $.ajax({
            url: "{{ route('admin.organizacion.guardar_orden') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                orden: orden
...
<main class="commerce-page">
    ...
    <section class="commerce-hero">
        <div class="commerce-hero__content">
            <h1 class="commerce-title">Ordenar organizaciones</h1>
                    ...
                    <table class="commerce-table">
                        <thead>
                            <tr class="commerce-table-head">
                                ...
                                <th style="width: 60px">ORDENAR</th>
                            </tr>
                        </thead>
                        <tbody id="lista-items">
                            @foreach($organizaciones as $organizacion)
                                <tr class="commerce-row item" data-id="{{ $organizacion->org_id }}">
