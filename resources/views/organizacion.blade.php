...
<main class="vp-brand-page">
    ...
    @php
        ...
        $logo = $organizacion->logoPrincipal?->of_img_path ? asset('storage/' . $organizacion->logoPrincipal->of_img_path) : '#';

        $heroImage = $organizacion->imagenPrincipal?->of_img_path ? asset('storage/' . $organizacion->imagenPrincipal->of_img_path) : false;

        $resaltador = $organizacion->resaltador_entidad;
    ...
    <section class="vp-brand-hero">
        <img src="{{ asset($heroImage) }}" alt="{{ $organizacion->org_nombre_fantasia ?? 'Comercio' }}">
    </section>
    @else
    <section class="vp-brand-hero" style="height: 12vh !important; {{ $organizacion->org_color_fondo!='' ? 'background: '.$organizacion->org_color_fondo.' !important' : '' }}"></section>
    @endif

    <section class="vp-brand-info" style="{{ $organizacion->org_color_fondo!='' ? 'background: '.$organizacion->org_color_fondo.' !important' : '' }}">
        <div class="vp-brand-shell vp-brand-info__inner">

            <div class="vp-brand-main">
                @if($logo)
                <div class="vp-brand-logo">
                    <img src="{{ asset($logo) }}" alt="{{ $organizacion->org_nombre_fantasia ?? 'Comercio' }}">
                </div>
                @else
                <div class="vp-brand-logo" style="{{ $organizacion->org_color_fondo!='' ? 'background: '.$organizacion->org_color_fondo.' !important; border: none !important;' : '' }}"></div>
                @endif
                ...
                <div>
                    ...
                    <h1>{{ $organizacion->org_nombre_fantasia ?? 'Nombre del comercio' }}</h1>

                    <p class="vp-brand-category">
                        {{ $organizacion->org_descripcion_publica ?? 'Comercio' }}
                    </p>
