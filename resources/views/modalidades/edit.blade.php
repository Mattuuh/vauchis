...
@php
    ...
    if (is_null($oldCampos)) {
        $oldCampos = $modalidad->campos->map(function ($campo) {
            return [
                'id' => $campo->mca_id,
                'codigo' => $campo->mca_codigo,
                'nombre' => $campo->mca_nombre,
                'tipo' => $campo->mca_tipo,
                'tipo_monto' => $campo->mca_tipo_numero,
                'label' => $campo->mca_label,
                'placeholder' => $campo->mca_placeholder,
                'requerido' => $campo->mca_requerido,
                'orden' => $campo->mca_orden,
                'opciones' => $campo->mca_opciones,
                'ayuda' => $campo->mca_ayuda,
                'estado' => $campo->mca_estado,
            ];
        })->toArray();
    }
@endphp
...
<div class="container">
    ...
    <section class="vch-hero">
        <div class="vch-hero__content">
            <h1 class="vch-title">Editar modalidad</h1>
            <p class="vch-subtitle">Modifica los datos de la modalidad y administra sus campos dinámicos.</p>
        </div>
    </section>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="f_tipo_mod_id" id="f_tipo_mod_id" class="form-select field-required">
                        ...
                        @foreach($tipos_modalidades as $tipo)
                            <option value="{{ $tipo['tipo_mod_id'] }}" {{ old('f_tipo_mod_id', $modalidad->tipo_mod_id) == $tipo['tipo_mod_id'] ? 'selected' : '' }}>
                ...
                @if(count($oldCampos))
                    @foreach($oldCampos as $i => $campo)
                        <div class="campo-dinamico-item border rounded p-3 mb-3 position-relative" data-index="{{ $i }}">
                            ...
                            @if(!empty($campo['id']))
                                <input type="hidden" name="campos[{{ $i }}][id]" value="{{ $campo['id'] }}">
                            @endif
                            ...
                            <div class="row g-3">
                                ...
                                {{-- <div class="col-12 campo-opciones-wrapper" style="{{ (($campo['tipo'] ?? '') === 'select') ? '' : 'display:none;' }}">
                                    <label class="form-label">Opciones a mostrar</label>
                                    <textarea name="campos[{{ $i }}][opciones]" class="form-control" rows="2" placeholder="Ej: ARS,USD o Bajo,Medio,Alto">
                                        {{ $campo['opciones'] ?? '' }}
                                    </textarea>
    ...
    <div class="campo-dinamico-item border rounded p-3 mb-3 position-relative" data-index="__INDEX__">
        ...
        <div class="row g-3">
            ...
            {{-- <div class="col-12 campo-opciones-wrapper" style="display:none;">
                ...
                <textarea name="campos[__INDEX__][opciones]" class="form-control" rows="2" placeholder="Ej: ARS,USD o Bajo,Medio,Alto"></textarea>
                <div class="form-text">Separá las opciones por coma.</div>
            </div>
...
<script>
    ...
    function normalizarCodigoModalidad(valor) {
        return valor
            .toUpperCase()
            .replace(/\s+/g, '_')
            .replace(/[^A-Z0-9_]/g, '');
    ...
    function bindCampoEvents(campoItem) {
        ...
        if (inputCodigo) {
            inputCodigo.addEventListener('input', function () {
                this.value = normalizarCodigoCampo(this.value);
            });
        }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (inputCodigoModalidad) {
            inputCodigoModalidad.addEventListener('input', function () {
                this.value = normalizarCodigoModalidad(this.value);
            });
        }
